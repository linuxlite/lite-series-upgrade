...
"""Lite Series upgrade utility.

This module contains a GTK application that orchestrates an upgrade from
Linux Lite 6.x to the 7.x release series.  The script was provided as part of a
support request and has been normalized to ensure consistent indentation and to
allow automated checks for syntax errors.
"""
...
from gi.repository import Gio, GLib, Gtk  # noqa: E402  (import after gi.require_version)
...
def run_cmd(cmd, env=None, shell=False, timeout=None, log_output=True, dry_run=False):
    ...
    try:
        ...
        process.kill()
        logger.exception("Error running %s: %s", cmd, exc)
        rc = 1
    if rc != 0:
...
class UpgradeEngine:
    ...
    def __init__(self, ui_callback, progress_callback, dry_run=False, reenable_ppas=False):
        ...
        self.plan = [
            (
                ...
                self.WEIGHTS["System check & preparation"],
                self.step_system_check,
            ),
            (
                "Disable third-party & update Linux Lite repo",
                self.WEIGHTS["Disable third-party & update Linux Lite repo"],
                self.step_disable_third_party,
            ),
            (
                "Fix broken packages & configure pending",
                self.WEIGHTS["Fix broken packages & configure pending"],
                self.step_fix_common,
            ),
            (
                "Ensure upgrade tools",
                self.WEIGHTS["Ensure upgrade tools"],
                self.step_ensure_tools,
            ),
            (
                "Release upgrade to 24.04",
                self.WEIGHTS["Release upgrade to 24.04"],
                self.step_release_upgrade,
            ),
            (
                "Auto-resolve common issues",
                self.WEIGHTS["Auto-resolve common issues"],
                self.step_auto_resolve,
            ),
            (
                "Install LibreOffice Series 7 bundle",
                self.WEIGHTS["Install LibreOffice Series 7 bundle"],
                self.step_install_libreoffice_bundle,
            ),
            (
                "Post-upgrade cleanup",
                self.WEIGHTS["Post-upgrade cleanup"],
                self.step_post_upgrade,
            ),
            (
                "Update branding/version files",
                self.WEIGHTS["Update branding/version files"],
                self.step_update_branding,
            ),
            (
                "Verify upgraded release",
                self.WEIGHTS["Verify upgraded release"],
                self.step_verify,
            ),
        ]
        if self.reenable_ppas:
            self.plan.append(
                (
                    "Re-enable known-good PPAs",
                    self.WEIGHTS["Re-enable known-good PPAs"],
                    self.step_reenable_known_ppas,
                )
            )
    ...
    def emit(self, text: str):
        logger.info(text)
        GLib.idle_add(self.ui_callback, f"{text} ")
    ...
    def step_system_check(self):
        ...
        if os.geteuid() != 0 or "PKEXEC_UID" not in os.environ:
            ...
            return False
        self._inc_progress(
            self.WEIGHTS["System check & preparation"], "System check complete"
        )
        return True
        ...
        for fname in ["linuxlite.list", "linuxlite.list.save"]:
            ...
            try:
                ...
                if "fluorite" in content:
                    ...
                    if self.dry_run:
                        self.emit(
                            f"[DRY RUN] Would back up {fname} -> {backup.name} and replace 'fluorite' with 'galena'"
                        )
                    else:
                        ...
                        self.emit(
                            f"Updated {fname}: fluorite -> galena (backup: {backup.name})"
                        )
                else:
                    self.emit(f"No 'fluorite' found in {fname}; no change needed")
            except Exception as exc:  # noqa: BLE001
                self.emit(f"Warning: could not update {fname}: {exc}")
    ...
    def step_disable_third_party(self):
        ...
        for f in d.glob("*.list"):
            try:
                ...
                self.emit(f"Warning: could not inspect {f}: {exc}")
    ...
    def step_fix_common(self):
        ...
        for i, cmd in enumerate(cmds):
            for line in run_cmd(cmd, env=self.env, dry_run=self.dry_run):
                self.emit(line)
            self._inc_progress(
                last_bonus if i == len(cmds) - 1 else per, "Fix/cleanup step done"
            )
    ...
    def step_ensure_tools(self):
        ...
        for i, pkg in enumerate(pkgs):
            for line in run_cmd(self._apt("install", pkg), env=self.env, dry_run=self.dry_run):
                self.emit(line)
            self._inc_progress(
                last_bonus if i == len(pkgs) - 1 else per, f"Ensured {pkg}"
            )
...
""")
                ...
                self.emit(
                    "[DRY RUN] Would set Prompt=lts in /etc/update-manager/release-upgrades"
                )
        except Exception as exc:  # noqa: BLE001
            self.emit(f"Warning: could not write release-upgrades config: {exc}")
        prep_weight = 1
    ...
    def step_auto_resolve(self):
        ...
        for i, cmd in enumerate(cmds):
            for line in run_cmd(cmd, env=self.env, dry_run=self.dry_run):
                self.emit(line)
            self._inc_progress(
                last_bonus if i == len(cmds) - 1 else per, "Auto-resolve step done"
            )
    ...
    def _download_file(self, url: str, dest: Path):
        if self.dry_run:
            ...
            return True
        try:
            with urllib.request.urlopen(url) as response, open(dest, "wb") as fileobj:
                shutil.copyfileobj(response, fileobj)
            return True
        except Exception as exc:  # noqa: BLE001
            self.emit(f"Error downloading {url}: {exc}")
            return False
    ...
    def _extract_tar_gz(self, tar_path: Path, out_dir: Path):
        ...
        try:
            ...
            self.emit(f"Error extracting {tar_path}: {exc}")
            return False
    ...
    def step_install_libreoffice_bundle(self):
        ...
        for f in apps_dir.glob("libreoffice7.5-*.desktop"):
            try:
                ...
                self.emit(f"Warning: could not remove {f}: {exc}")
    ...
    def step_post_upgrade(self):
        ...
        for i, cmd in enumerate(cmds):
            for line in run_cmd(cmd, env=self.env, dry_run=self.dry_run):
                self.emit(line)
            self._inc_progress(
                last_bonus if i == len(cmds) - 1 else per, "Post-upgrade step done"
            )
    ...
    def _replace_in_file(self, path: Path, replacements: list[tuple[str, str]]):
        try:
            ...
            self.emit(f"Warning: could not update {path}: {exc}")
            return False
    ...
    def step_update_branding(self):
        ...
        changed += bool(
            self._replace_in_file(
                Path("/etc/llver"),
                [(f"Linux Lite {LL_FROM_VERSION}", f"Linux Lite {LL_TO_VERSION}")],
            )
        )
        changed += bool(
            self._replace_in_file(
            ...
            self._replace_in_file(
                ...
                [
                    (
                        ...
                        f'DISTRIB_DESCRIPTION="Linux Lite {LL_TO_VERSION}"',
                    )
                ],
            )
        )
            ...
            self._replace_in_file(
                ...
                [
                    (
                        ...
                        f'PRETTY_NAME="Linux Lite {LL_TO_VERSION}"',
                    )
                ],
            )
        )
        changed += bool(
            self._replace_in_file(
                Path("/usr/share/plymouth/themes/text.plymouth"),
                [
                    (
                        f"title=Linux Lite {LL_FROM_VERSION}",
                        f"title=Linux Lite {LL_TO_VERSION}",
                    )
                ],
            )
        )
    ...
    def step_verify(self):
        if self.dry_run:
            ...
            self.emit(
                "[DRY RUN] Would update /etc/llver, /etc/issue, /etc/lsb-release, /etc/os-release, and text.plymouth with new version strings"
            )
            self._inc_progress(
                self.WEIGHTS["Verify upgraded release"],
                "Verification & branding skipped (dry run)",
            )
            return True
        ok = False
        ...
        try:
            ...
            self.emit(f"Warning: could not update /etc/llver: {exc}")
        try:
            Path("/etc/issue").write_text("Linux Lite 7.6 LTS \n \l")
            self.emit("Updated /etc/issue → Linux Lite 7.6 LTS")
        except Exception as exc:  # noqa: BLE001
            self.emit(f"Warning: could not update /etc/issue: {exc}")
        try:
            ...
            self.emit(f"Warning: could not update /etc/lsb-release: {exc}")
        try:
            ...
            self.emit(f"Warning: could not update /etc/os-release: {exc}")
        try:
            ...
            self.emit(f"Warning: could not update plymouth theme: {exc}")

        self._inc_progress(
            self.WEIGHTS["Verify upgraded release"], "Verification & branding complete"
        )
        if ok:
    ...
    def step_reenable_known_ppas(self):
        ...
        if not to_consider:
            ...
            self._inc_progress(
                self.WEIGHTS["Re-enable known-good PPAs"], "No PPAs to re-enable"
            )
            return True
        count = 0
        for f in to_consider:
            ...
            if not content_path.exists():
                self.emit(
                    f"Warning: could not locate {f.name} or {target.name}; skipping"
                )
                continue
            try:
                txt = content_path.read_text()
            except Exception as exc:  # noqa: BLE001
                self.emit(f"Warning: could not process {content_path.name}: {exc}")
                continue
            if any(s in txt for s in self.KNOWN_PPA_WHITELIST):
                if self.dry_run:
                    ...
                    if target.exists():
                        ...
                        self.emit(
                            f"Warning: expected {f.name} to exist; skipping"
                        )
                        continue
                    else:
                        f.rename(target)
        ...
        for _ in run_cmd(self._apt("update"), env=self.env, dry_run=self.dry_run):
            pass
        self._inc_progress(
            self.WEIGHTS["Re-enable known-good PPAs"], f"Re-enabled {count} PPAs"
        )
        return True
...
class MainWindow(Gtk.ApplicationWindow):
    def __init__(self, app):
        ...
        header = Gtk.Label(
            label=f"{APP_NAME} Upgrade Linux Lite Series 6 → Linux Lite Series 7"
        )
        ...
        self.reenable_ppas_cb = Gtk.CheckButton.new_with_label(
            "Re-enable known-good PPAs after upgrade"
        )
    ...
    def on_start(self, _btn):
        ...
        def worker():
            engine = UpgradeEngine(
                self.append_text, self.update_progress, dry_run=dry_run, reenable_ppas=reenable
            )
            ok = engine.run()
    ...
    def on_view_log(self, _btn):
        ...
        try:
            ...
            dlg = Gtk.MessageDialog(
                ...
                text=f"Log path: {path}",
            )
            dlg.connect("response", lambda dialog, _resp: dialog.destroy())
            dlg.present()
...
class LiteSeriesUpgradeApp(Gtk.Application):
    ...
    def do_activate(self, *args):  # noqa: D401  (Gtk override, signature defined by API)
...
def main():
    """Script entry point enforcing pkexec usage."""
